...
- **后端 `batcher.py`**：任务队列 + `ThreadPoolExecutor` 并行 Worker Pool
  - 并行数可调（1~16），按电脑性能规划任务量
  - Qt 信号驱动：`task_started` / `task_completed` / `task_failed`
  - 任务状态管理：等待 / 处理中 / 完成 / 失败 / 跳过
  - 支持拖拽调整处理顺序
  - 开始 / 停止控制
- **GUI 批量标签页**：侧边栏新增「📋 批量」按钮
  - 文件列表表格（序号 / 文件名 / 状态 / 用时）
  - 拖拽排序
  - 并行数 SpinBox
  - 全局进度条 + 逐文件状态刷新
...
- **导出文稿可选时间戳**：`--no-timestamp` CLI 标志 + GUI 导出弹窗复选框
- **README 全面翻新**：免安装下载指引 + GUI 介绍 + 打包说明
